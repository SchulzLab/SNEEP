...
The interactions from different sources can overlap and can be linked to the same and to different genes.                                  

Table \@ref(tab:associatedGenes) holds the target genes of all SNPs effecting a TF for which a motif was provided and overlapping with a interaction.

For instance, if we consider the first row of Table \@ref(tab:associatedGenes), `r genes_table$numberSNPs[[1]]` SNPs significantly affect the binding behavior of TFs and overlap with  `r genes_table$numberREMs[[1]]` interaction`r if(as.double(genes_table$numberREMs[[1]]) >1){"s"}`  associated to the gene `r genes_table$gene[[1]]` (ensembl ID: `r genes_table$ensemblID[[1]]`). 
...
```{r, echo = FALSE}
	genes_table$TFs = gsub("..", "::", genes_table$TFs, fixed=TRUE )
	genes_table$TFs = gsub(".M", "(M", genes_table$TFs, fixed=TRUE )
	genes_table$TFs = gsub(".,", "),", genes_table$TFs, fixed=TRUE )
	genes_table$TFs = linebreak(genes_table$TFs)
	genes_table$SNPs = linebreak(genes_table$SNPs)
	genes_table$gene = gsub("_", "\\_", genes_table$gene, fixed=TRUE) #necessary since escape  must be false for linebreaking and the gene names can include _ which must be especially handled in latex (usually escape is taking care of this)

	knitr::kable(x = genes_table, format = "latex",longtable = T, booktabs = T, col.names = c( "gene name", "ensembl ID", "number\ndifferent interactions", "number\ndifferent SNPs", "SNPs", "TFs" ), align = "llllll", linesep = "", caption = "Target genes of the SNPs with a significant change in the binding affinity of a TF.",  row.names = F, escape = F, label = "associatedGenes") %>%
	...
	collapse_rows(columns = 1:4, target = 1, valign = "top") #%>%
