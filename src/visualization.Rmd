...
if (associatedTFs == TRUE){
	...
	entries = data.frame(matrix(0L, nrow = numTFs, ncol = 5)) #initalize matrix with 0
	names_sig_TFs = rownames(sig_TFs)
	#rownames(entries) = names_sig_TFs
	rownames(entries) = TF_names
	...
	entries = subset(entries, pvaluesBinomial >0) #eliminate TFs which do not occur at all
	entries = subset(entries, pvaluesBinomial <=0.1) # print only sig gain or loss TFs
	#print(entries)
...
#genes
genes_table = data.frame(matrix(ncol = 6))
colnames(genes_table) = c("gene", "ensemblID", "numberREMs", "numberSNPs", "SNPs", "TFs")

if (associatedGenes == TRUE){ #there are  associated genes otherwise skip this
	genes = genes[-1,] #remove first row (contains only NAs)
	genes = ddply(genes,.(gene, ensemblID),nrow) #determine how often a gene occurs
	...
	for (row in 1:nrow(gene_TFs)){
		...
		SNP = gene_TFs[row, "SNP"]
		TF = gene_TFs[row, "TF"]
		if (gene == previous_gene || previous_gene == ""){
			if (has.key(SNP, TF_SNPs)){ #is SNP already in TF_SNPs?
				h = TF_SNPs[[SNP]] #already associated TFs
				if (!(TF %in% h)){ #have we seen the current TF already?
					h = cbind(h, TF)
					TF_SNPs[[SNP]] = h
				}
			}else{
				counterSNPs = counterSNPs + 1 # count different SNPs per gene
				TF_SNPs[[SNP]] = c(TF)
			}
			...
			k = keys(TF_SNPs) #give all SNPs per gene
			#firstElem = TRUE
			for (elem in k){
				#store all TFs in a list for one SNP
				h = TF_SNPs[[elem]] 
				firstTime = TRUE
				TF_list = ""
				for (e in h){
					if (firstTime == TRUE){
						TF_list = paste0(TF_list, e)
						firstTime = FALSE
					}else{
						TF_list = paste0(TF_list, ", " , e)
					}
				}	
				ID = genes[previous_gene, "ensemblID"]
				n = genes[previous_gene, "gene"]
				genes_table = rbind(genes_table, c(n, ID, counter, counterSNPs, elem, TF_list))
			}
			previous_gene = gene
			TF_SNPs = hash()
			TF_SNPs[[SNP]] = TF
	...
	genes_table = genes_table[-1,] #remove first row (contains only NAs)
	genes_table = genes_table[order(strtoi(genes_table$numberSNPs), decreasing = TRUE),]


	REMs_table = data.frame(matrix(ncol = 7))
	colnames(REMs_table) = c("regionID", "length", "gene", "ensemblID",  "numberSNPs", "SNPs", "TFs")
	#REMs
	...
	for (row in 1:nrow(REM_TFs)){
		...
		if (REM == previous_REM || previous_REM == ""){
			...
			for (elem in k){
				...
				ID = REMs[previous_REM, "ensemblID"]
				n = REMs[previous_REM, "gene"]
				length = REMs[previous_REM, "length"]
				REMs_table = rbind(REMs_table, c(REM, length, n, ID,  counterSNPs, elem, TF_list))
			}
			previous_REM = REM
			TF_SNPs = hash()
			TF_SNPs[[SNP]] = TF
	...
	REMs_table = REMs_table[-1,] #remove first row (contains only NAs)
	REMs_table = REMs_table[order(strtoi(REMs_table$numberSNPs), decreasing = TRUE),]

	allREMs = REMs
	REMs_table = subset(REMs_table, numberSNPs >=2) #only show rems associatedd to more than 1 SNP
	associatedREMs = FALSE
	if (nrow(REMs_table) != 0){
		associatedREMs = TRUE
	}
}
